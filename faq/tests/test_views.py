...
def test_faq_api_with_language():
    ...
    # ✅ Fix: Ensure Bengali translation exists instead of assuming English
    response = client.get(reverse('faq-list') + '?lang=bn')
    assert response.status_code == 200
    assert response.data[0]['question'] is not None  # Ensure translation exists
    assert response.data[0]['answer'] is not None  # Ensure translation exists
